...
static uint8_t telemetry_payload[150];
static uint32_t telemetry_send_count = 0;

DHT dht(D1,tipoDHT);  //Istanzia l'oggetto dht della classe DHT    
static double h;          //umidità   
static double t;         //temperatur
static double g; //gas
...
{
  char* curr_time=getCurrentLocalTimeString();
  ...
  (void)az_span_dtoa(temp_span, t, 2, &temp_span);
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" , \"humidity\": "));
  (void)az_span_dtoa(temp_span, h, 0, &temp_span);
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" , \"gas_perc\": "));
  (void)az_span_dtoa(temp_span, g, 2, &temp_span);
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" , \"device\": "));
  (void)az_span_u32toa(temp_span, devid, &temp_span);
  /*temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \"timestamp\": "));
  temp_span = az_span_copy(temp_span, az_span_create_from_str(curr_time));*/
