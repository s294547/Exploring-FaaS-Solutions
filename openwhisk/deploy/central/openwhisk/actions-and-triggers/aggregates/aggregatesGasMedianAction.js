    ...
    request(requestOptions, function (error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        rows.forEach(row => {
          ...
          columns.shift();
          if(j==0 && columns[0]=="median"){
            location=columns[5];
            j++;
          }
          if(columns[0]=='median'){
            if(columns[4]=='gas_perc')
              gas_perc_median= columns[6];
          }
        });
        ...
        var influxData=`gas_perc,datacenter_id=${location},year=${year},month=${month},day=${day} median=${gas_perc_median} ${tday}\n`;
